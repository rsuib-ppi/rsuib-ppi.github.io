var rsAPI =
  "https://script.google.com/macros/s/AKfycbwRL_FP_WfkPX2nVitQn-O6R4LV4HOdwjQGXlwAYVm4T7IkGUYBFZu_DywPUfgfrNT_7w/exec";
var dbAPI =
  "https://script.google.com/macros/s/AKfycbzkPctTQ0nX5P4RQ2f3314uvroiI1VuXJZplwU42r2jzHkX2Ptklh0VdBjxyLbqF6UE/exec";
var sendform = {
  userName: "",
  userAgent: "",
};
...
async function NavbarTo(target) {
  ...
  if (target === "Supervisi") {
    Elem("bodyContent").setAttribute(
      "w3-include-html",
      "/html/C3-supervisi.html"
    );
  ...
  let response = await fetch(
    Elem("bodyContent").getAttribute("w3-include-html")
  );
  ...
  document.querySelectorAll(".input-with-list").forEach((elem) => {
    ...
    function filterInputList() {
      ...
      if (!Elem(inputID).classList.contains("no-list-add")) {
        var listTambah = document
          .querySelector("#tambah-list-template > ul")
          .cloneNode(true);
      }

      var filteredData = database[
        listElem.getAttribute("list-data-to-filter")
      ].filter((x) => {
        var j = false;
        ...
        return (
          x["name"].toString().toLowerCase().includes(text.toLowerCase()) || j
        );
      });
      ...
      if (text.length > 0) {
        if (!Elem(inputID).classList.contains("no-list-add")) {
          ...
          document.querySelector(
            "#" + listTargetID + " > ul > .tambah-list > div > span"
          ).innerHTML = text;
          document
            .querySelector("#" + listTargetID + " > ul > .tambah-list > div")
            .setAttribute(
              "onclick",
              listElem
                .getAttribute("tambah-list-function")
                .replace(
                  "()",
                  "('" +
                    text +
        ...
        if (filteredData.length > 0) {
          ...
          for (var i = 0; i < filteredData.length; i++) {
            ...
            var addFunc1 = "";
            if (
              Elem(listTargetID).getAttribute("list-data-to-filter") ==
              "staffData"
            ) {
              addFunc1 =
                "SelectStaffBase('" +
                filteredData[i].baseGroup +
                "', '" +
                filteredData[i].baseUnit +
                "'); ";
            }
            liName.setAttribute(
              "onclick",
            ...
            if (Elem(inputID).classList.contains("no-list-add")) {
              ...
              liEdit.setAttribute(
                "onclick",
                Elem(inputID).getAttribute("edit-function") +
                  "(" +
                  filteredData[i].id +
                  ")"
              );
...
function SelectStaffBase(group, unit) {
  if (!(group === "")) {
    document.querySelectorAll(".input-kelompok").forEach((p) => {
      p.value = group;
    });
  } else {
    document.querySelectorAll(".input-kelompok").forEach((p) => {
  ...
  document.querySelectorAll(".input-unit").forEach((p) => {
    p.value = unit;
  });
}
...
function inputChange(elem) {
  ...
  if (inputGroup === "hh") {
    hhInputChange(elem);
  } else if (inputGroup === "apd") {
    apdInputChange(elem);
  }
  // console.log(inputGroup)
}
async function EditStaff(id) {
  var ids = database.staffData.map((p) => {
    return p.id;
  });
...
async function EditUnit(id) {
  var ids = database.unitData.map((p) => {
    return p.id;
  });
  ...
  Object.keys(formAssignObj).forEach((p) => {
    var units = formAssignObj[p].map((p) => {
      return p.name;
    });
...
async function staffAPI(code) {
  if (confirm(code + " data?") == true) {
    ...
    var nama = staffDataModal
      .querySelector("#staff-input-nama")
      .value.toString()
      .toUpperCase();
    var groupBase = staffDataModal.querySelector("#staff-input-kelompok").value;
    var unitBase = staffDataModal
      .querySelector("#staff-input-unit")
      .value.toString()
      .toUpperCase();
    if (code == "Tambah") {
...
async function unitAPI(code) {
  if (confirm(code + " data?") == true) {
    ...
    var unitName = myModal
      .querySelector("#unit-input-nama")
      .value.toString()
      .toUpperCase();
    var unitAltName = myModal
      .querySelector("#unit-input-alternate")
      .value.toString()
      .toUpperCase();
